...
export class PublishChangesetHandler implements ICommandHandler<PublishChangesetCommand> {
  ...
  async execute(command: PublishChangesetCommand): Promise<void> {
    ...
    const changeset = await this.em.findOneOrFail(
      AccountChangeset, 
      command.changesetId, 
      { populate: ['newSnapshots', 'newSnapshots.node'] } // Traz os nós associados
    );
    ...
    for (const snapshot of changeset.newSnapshots) {
      // Como usamos Rel<T> e fizemos o populate no findOneOrFail, 
      // snapshot.node já é a entidade AccountNode pronta para uso.
