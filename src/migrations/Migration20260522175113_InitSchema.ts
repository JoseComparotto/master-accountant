...
export class Migration20260522175113_InitSchema extends Migration {

  override up(): void | Promise<void> {
    this.addSql(`create schema if not exists "coa";`);
    this.addSql(`create table "coa"."chart_of_accounts" ("id" uuid not null, "name" varchar(255) not null, "level_widths" integer[] not null, "is_active" boolean not null default true, "created_at" timestamptz not null, primary key ("id"));`);

    this.addSql(`create table "coa"."account_changeset" ("id" uuid not null, "status" text not null default 'draft', "chart_of_accounts_id" uuid not null, "increment_type" text not null, "effective_date" date null, "created_at" timestamptz not null, "published_at" date null, "discarded_at" date null, primary key ("id"));`);

    this.addSql(`create table "coa"."account_node" ("id" uuid not null, "chart_of_accounts_id" uuid not null, "parent_id" uuid null, "is_abstract" boolean not null, "node_code" integer not null, "path_ltree" ltree not null, "formatted_code" varchar(255) not null, "current_snapshot_id" uuid null, "creation_changeset_id" uuid not null, primary key ("id"));`);
    ...
    this.addSql(`create table "coa"."account_transition" ("id" uuid not null, "changeset_id" uuid not null, "status" text not null, "transition_type" text not null, "source_node_id" uuid null, "target_node_id" uuid null, "change_reason" varchar(255) null, "created_at" timestamptz not null, "effective_date" date null, primary key ("id"));`);

    this.addSql(`create table "coa"."account_snapshot" ("id" uuid not null, "node_id" uuid not null, "changeset_id" uuid not null, "status" text not null, "previous_snapshot_id" uuid null, "name" varchar(255) not null, "balance_type" text not null, "account_class" text not null, "change_reason" varchar(255) null, "created_at" timestamptz not null, "effective_date" date null, primary key ("id"));`);
    ...
    this.addSql(`alter table "coa"."account_node" add constraint "account_node_creation_changeset_id_foreign" foreign key ("creation_changeset_id") references "coa"."account_changeset" ("id");`);
    ...
    this.addSql(`alter table "coa"."account_transition" add constraint "account_transition_status_check" check ("status" in ('draft', 'published', 'discarded'));`);
    ...
    this.addSql(`alter table "coa"."account_snapshot" add constraint "account_snapshot_status_check" check ("status" in ('draft', 'published', 'discarded'));`);
  ...
  override down(): void | Promise<void> {
    ...
    this.addSql(`alter table "coa"."account_node" drop constraint "account_node_creation_changeset_id_foreign";`);
